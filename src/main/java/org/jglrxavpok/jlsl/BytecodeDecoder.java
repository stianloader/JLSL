...
public class BytecodeDecoder extends CodeDecoder {
	...
	public void handleClass(ClassNode classNode, List<CodeFragment> out) {
		...
		for (MethodNode node : methodNodes) {
			...
			String[] argsTypes = typesFromDesc(
					node.desc.substring(node.desc.indexOf('(') + 1, node.desc.indexOf(')')));
			int argIndex = 0;
	...
	private static void handleMethodNode(MethodNode node, HashMap<Integer, String> varTypeMap,
			HashMap<Integer, String> varNameMap, List<CodeFragment> out) {
		int lastFrameType = 0;
		int frames = 0;
		int framesToSkip = 0;
		...
		for (int index = 0; index < instructions.size(); index++) {
			...
			if (ainsnNode.getType() == AbstractInsnNode.INSN) {
				...
				else if (ainsnNode.getOpcode() == LRETURN || ainsnNode.getOpcode() == DRETURN
						|| ainsnNode.getOpcode() == FRETURN || ainsnNode.getOpcode() == IRETURN
				...
				} else if (ainsnNode.getOpcode() == DREM || ainsnNode.getOpcode() == IREM
						|| ainsnNode.getOpcode() == FREM || ainsnNode.getOpcode() == LREM) {
				...
				else if (ainsnNode.getOpcode() == AASTORE || ainsnNode.getOpcode() == IASTORE
						|| ainsnNode.getOpcode() == BASTORE || ainsnNode.getOpcode() == LASTORE
						|| ainsnNode.getOpcode() == SASTORE || ainsnNode.getOpcode() == FASTORE
				...
				if (jumpNode.getOpcode() == IFEQ) {
					if (instructions.get(index - 1).getOpcode() == ILOAD
				...
				} else if (jumpNode.getOpcode() == IF_ICMPEQ && instructions.get(index + 1).getOpcode() == ICONST_1
						&& (instructions.get(index + 2).getOpcode() == IRETURN
								|| instructions.get(index + 2).getOpcode() == ISTORE)
						...
						&& (instructions.get(index + 6).getOpcode() == IRETURN
								|| instructions.get(index + 2).getOpcode() == ISTORE)) {
					NotEqualCheckFragment notEqualFrag = new NotEqualCheckFragment();
					out.add(notEqualFrag);
					index += 5;
				} else if (jumpNode.getOpcode() == IF_ICMPNE && instructions.get(index + 1).getOpcode() == ICONST_1
						&& (instructions.get(index + 2).getOpcode() == IRETURN
								|| instructions.get(index + 2).getOpcode() == ISTORE)
						&& instructions.get(index + 3).getType() == AbstractInsnNode.LABEL
						&& instructions.get(index + 4).getType() == AbstractInsnNode.FRAME
				...
				if (ainsnNode.getOpcode() == ISTORE) {
					...
					out.add(storeFrag);
				} else if (ainsnNode.getOpcode() == FLOAD || ainsnNode.getOpcode() == LLOAD
						|| ainsnNode.getOpcode() == ILOAD || ainsnNode.getOpcode() == DLOAD
